...
import { getSpotifyQueryParams } from "./get-spotify-query-params";
...
export interface SpotifySelectionModalHandle {
  show: () => void;
  hide: () => void;
}
...
const availableImports = ["Profile", "Albums", "Audiobooks", "Episodes", "Following", "Playlists", "Shows", "Tracks", "Top Artists", "Top Tracks"];
const SpotifySelectionModal = forwardRef<SpotifySelectionModalHandle>(function SpotifySelectionModal(props, ref) {
  ...
  const handleImportClick = async () => {
    localStorage.setItem("can_fetch_spotify", "true");
    ...
    if (accessToken && accessToken.length > 0 && expiresAt && expiresAt.length > 0 && Number(expiresAt) > moment().unix() * 1000) {
      ...
      const queryParamsString = await getSpotifyQueryParams();
      router.push(`${env.NEXT_PUBLIC_SPOTIFY_AUTH_URL}?${queryParamsString}`);
    }
  };
  ...
  return (
    <>
      <div
        data-testid="cy-spotify-modal"
        className={mergeClassnames(styles.modalBackground, isShowing ? "flex" : "hidden")}>
        <div className={styles.modalContainer}>
          <p className={styles.modalTitle}>Import from Spotify</p>
          ...
          <form>
            ...
            <div className={styles.importAndCancelButtonContainer}>
              <button
                data-testid="cy-import-spotify-btn"
...
export default SpotifySelectionModal;
